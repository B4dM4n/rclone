...
func (fh *ReadFileHandle) openPending() (err error) {
	...
	o := fh.file.getObject()
	r, err := chunkedreader.NewWithChunkSizeIterator(context.TODO(), o, fh.file.d.vfs.chunkSizeFunc()).Open()
	if err != nil {
		return err
	}
...
func (fh *ReadFileHandle) seek(offset int64, reopen bool) (err error) {
	...
	if !reopen {
		...
		// re-open with a seek
		o := fh.file.getObject()
		r = chunkedreader.NewWithChunkSizeIterator(context.TODO(), o, fh.file.d.vfs.chunkSizeFunc())
